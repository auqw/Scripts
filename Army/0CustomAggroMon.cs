...
public class CustomAggroMon
{
    ...
    public void MakeAggroMon()
    {
        ...
        {
            if (
                int.TryParse(s, out int monID) && Bot.Monsters.TryGetMonster(monID, out var Monster)
            )
            {
                if (Monster != null)
                {
        ...
        {
            Core.Logger(
                $"The bot was unable to find the following monster{(remainder2.Count == 1 ? "" : "s")}: "
                    + string.Join('|', remainder2)
            );
        }

        if (monNames.Count == 0)
            Core.Logger(
                "No monsters were found based on your input. The bot will now stop.",
                messageBox: true,
                stopBot: true
            );
        ...
        string getMonsters()
        {
            ...
            _monData = _monData
                .OrderByDescending(m => _monData.Count(_m => _m.ID == m.ID))
                .ToList();
            ...
            foreach (var _mon in _monData)
            {
                ...
                if (!_monsters.Contains(data))
                {
                    ...
                    foreach (
                        string cell in _monData.Where(m => m.ID == _mon.ID).Select(m => m.Cell)
                    )
                        if (!_cells.Contains(cell))
                            _cells.Add(cell);

                    _monsters.Add(
                        $"     Cell count: {_cells.Count}{(_mon.Race == "None" ? string.Empty : $", Race-Type: {_mon.Race}")}, HP: {_mon.MaxHP}"
                    );
                }
            }

            InputDialogViewModel monDiag = new(
                "Monsters in /" + map,
        ...
        void GenerateFile()
        {
            ...
            if (
                !Directory.Exists(
                    Path.Combine(ClientFileSources.SkuaScriptsDIR, "Army", "Generated")
                )
            )
            ...
            if (questIDs.Count > 0)
            {
                ...
                template[questIndex] =
                    $"{spaces}private List<int> questIDs = new() {{ {string.Join(", ", questIDs)} }};";
            }
            int monsterIndex = FetchIndex("private static readonly string[] monNames={ };");
            template[monsterIndex] =
                $"{spaces}private List<string> monNames = new() {{ \"{string.Join("\", \"", monNames)}\" }};";

            if (drops.Count > 0)
            {
                int dropsIndex = FetchIndex("private static readonly string[] drops={ };");
                template[dropsIndex] =
                    $"{spaces}private List<string> drops = new() {{ \"{string.Join("\", \"", drops)}\" }};";
            }
            ...
            template[classTypeIndex] =
                $"{spaces}private ClassType classtype = ClassType.{Bot.Config.Get<ClassType>("classtype")};";
            ...
            Core.WriteFile(
                Path.Combine(
                    ...
                    "Army",
                    "Generated",
                    diag.DialogTextInput.Replace(" ", "") + ".cs"
                ),
                template
            );
            Core.Logger(
                $"\"{diag.DialogTextInput.Replace(" ", "")}.cs\" has been generated and can be found in Skua_Modules/Scripts/Army/Generated",
                messageBox: true
            );
