...
public class ContractExchanging
{
    ...
    public List<IOption> Options = new()
    {
        ...
        new Option<bool>(
            "MaxorChoose",
            "Max bags or Choose",
            "Either choose to max the quest rewards or choose your reward manualy [true: max, false: choose]",
            true
        ),
        new Option<ContractExchangeRewards>(
            "RewardSelect",
            "Choose Your Quest Reward",
            "Select Your Quest Reward for Contract Exchange.",
            ContractExchangeRewards.All
        ),
        new Option<bool>(
            "Tainted Gem",
            "Farm Tainted Gem",
            "Farm \"Tainted Gem\" to max stack",
            true
        ),
        new Option<bool>(
            "Dark Crystal Shard",
            "Farm Dark Crystal Shard",
            "Farm \"Dark Crystal Shard\" to max stack",
            true
        ),
        new Option<bool>(
            "Gem of Nulgath",
            "Farm Gem of Nulgath",
            "Farm \"Gem of Nulgath\" to max stack",
            true
        ),
        new Option<bool>(
            "Blood Gem of the Archfiend",
            "Farm Blood Gem of the Archfiend",
            "Farm \"Blood Gem of the Archfiend\" to max stack",
            true
        ),
    };

    string[] ContractExchangeItems =
    {
    ...
    public void ContractExchange()
    {
        ...
        bool rewardSelectAll =
            Bot.Config!.Get<ContractExchangeRewards>("RewardSelect") == ContractExchangeRewards.All;
        ...
        if (maxOrChoose)
            ...
            if (
                ...
                Core.Logger(
                    "Options conflict: Max bags is true, but ChooseReward must be set to All, and the last 4 options must be true. Setting MaxorChoose to false.",
                    stopBot: true
                );

        foreach (
            ContractExchangeRewards reward in Enum.GetValues(typeof(ContractExchangeRewards))
                .Cast<ContractExchangeRewards>()
                .Where(r => r != ContractExchangeRewards.All)
        )
        {
            string? rewardName = Enum.GetName(typeof(ContractExchangeRewards), reward)
                ?.Replace("_", " ");
            ...
            if (
                maxOrChoose
                && (!taintedGem || !darkCrystalShard || !gemOfNulgath || !bloodGemOfArchfiend)
            )
                Core.Logger(
                    "Options conflict: Max bags is true, but one of the last 4 options is false.",
                    stopBot: true
                );
