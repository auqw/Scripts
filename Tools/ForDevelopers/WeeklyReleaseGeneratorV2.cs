...
//cs_include Scripts/Tools/ForDevelopers/DropPacketCollector.cs
//cs_include Scripts/Tools/ForDevelopers/LocationSwfQuestReader.cs
//cs_include Scripts/Tools/ForDevelopers/QuestPacketCollector.cs
...
public class WeeklyReleaseGeneratorV2
{
    ...
    private void Generate()
    {
        ...
        List<Quest> mapQuests = LoadMapQuests(questIDs, allQuests);
        ...
        foreach (string stale in Directory.GetFiles(outputDirectory, "Story*.cs")
            .Where(file => Regex.IsMatch(Path.GetFileName(file), @"^Story\d+\.cs$", RegexOptions.IgnoreCase)))
            File.Delete(stale);
        string repeatablesPath = Path.Combine(outputDirectory, "repetableQuests.cs");
        if (File.Exists(repeatablesPath))
            File.Delete(repeatablesPath);
        ...
        for (int index = 0; index < chains.Count; index++)
        {
            ...
            string className = $"Story{index + 1}";
    ...
    private List<Quest> LoadMapQuests(IReadOnlyList<int> questIDs, IReadOnlyList<Quest> localQuests)
    {
    ...
    )
    {
        ...
        IReadOnlyList<int> mapObjects = mapObjectsByQuest.TryGetValue(quest.ID, out IReadOnlyList<int>? objects)
            ? objects
            : Array.Empty<int>();
        List<MapItemSource> currentMapItems = mapObjects
            .Select(id => new MapItemSource(id, map, CurrentMapVerified: true))
            .ToList();
        List<MapItemSource> knownMapItems = currentMapItems.ToList();
        if (existingMapItemsByQuest.TryGetValue(quest.ID, out IReadOnlyList<MapItemSource>? existing))
            knownMapItems.AddRange(existing.Where(item => knownMapItems.All(known => known.ID != item.ID)));
        List<int> nonlocalIndexes = Enumerable.Range(0, requirements.Count)
            .Where(index => localDrops[index] == null)
            .ToList();
        ...
        else
        {
            ...
            HashSet<int> unresolvedIndexes = nonlocalIndexes
                .Except(mappedIndexes)
                .ToHashSet();
            List<int> unresolvedMapItemIndexes = unresolvedIndexes
                .Where(likelyMapItemIndexes.Contains)
                .ToList();
    ...
    )
    {
        ...
        {
            List<int> group = huntIndexes
                .Where(index => localDrops[index] != null && ClassType(localDrops[index]!) == classType)
                .ToList();
            if (group.Count == 0)
                continue;
        ...
        foreach (string file in Directory.GetFiles(scripts, "*.cs", SearchOption.AllDirectories))
        {
            ...
            foreach (Match method in Regex.Matches(source, @"public\s+void\s+(?<name>\w+)\s*\((?<args>[^)]*)\)\s*\{"))
            {
                if (!HasOnlyOptionalArguments(method.Groups["args"].Value))
                    continue;
                int end = FindClosingBrace(source, method.Index + method.Length - 1);
                if (end < 0)
                    continue;
    ...
    )
    {
        ...
        HashSet<int> firstDrops = sameName[0].Items.Select(item => item.ID).ToHashSet();
