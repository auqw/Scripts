...
public class WeeklyReleaseGenerator
{
    ...
    public void Generator()
    {
        ...
        List<string> monsterNames = Bot
            .Monsters.MapMonsters.Select(m => m.Name)
            .Distinct()
            .ToList();
        ...
        var sortedQuests = Core.EnsureLoad(Core.FromTo(startID, endID)).OrderBy(q => q.ID).ToList();
        ...
        foreach (Quest q in sortedQuests)
        {
            ...
            if (
                (q.Description?.IndexOfAny(mapItemKeywords.SelectMany(s => s).ToArray()) ?? -1) >= 0
            ...
            if (q.Requirements.Count > 0)
            {
                ...
                foreach (var req in q.Requirements)
                {
                    ...
                    {
                        generatedLines.Add(
                            $"    Story.MapItemQuest({q.ID}, \"{mapName}\", FillmeIn);"
                        );
                        addedMapItemQuest = true;
                ...
                if (
                    q.Requirements.Any(r =>
                        !mapItemKeywords.Any(k => (r.Name?.ToLower() ?? "").Contains(k))
                    )
                )
                {
                    ...
                    foreach (var req in q.Requirements.Select((r, i) => new { r, i }))
                    {
                        ...
                        generatedLines.Add(
                            $"        (\"{mapName}\", UseableMonsters[0], ClassType.Solo),"
                        );
                        Core.Logger(
                            $"🗡️ Requirement parsed: '{req.r.Name}' → UseableMonsters[0] → ClassType.Solo"
                        );
                    }
