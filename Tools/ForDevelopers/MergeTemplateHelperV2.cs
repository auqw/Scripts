...
public class MergeTemplateHelperV2
{
    ...
    )
    {
        ...
        List<GeneratedCase> cases = ingredients
            ...
            .ToList();
        List<(Quest Target, Quest Predecessor)> unlocks = new();
        HashSet<int> inspectedUnlockQuests = new();
        ...
        lines.AddRange(ingredients.Select(item => $"            \"{Escape(item.Name)}\","));
        lines.AddRange(new[]
        {
            "        ]);",
            "        Core.SetOptions();",
    ...
    private static string GenerateQuestRewardBody(Quest target, GenerationContext context)
    {
        ...
        HashSet<int> registeredQuestIDs = new();
        CollectRegisteredQuests(target, context, registeredQuestIDs, new HashSet<int>());
        if (registeredQuestIDs.Count > 0)
        {
            foreach (Quest quest in context.MapQuests.Where(quest => registeredQuestIDs.Contains(quest.ID)))
                foreach (ItemBase requirement in quest.AcceptRequirements ?? new List<ItemBase>())
                    EmitRequirement(lines, requirement, quest, context, new HashSet<int>(), "                    ", null);
        ...
        if (registeredQuestIDs.Contains(target.ID))
            EmitQuestRequirements(lines, target, context, new HashSet<int> { target.ID }, "                        ", registeredQuestIDs);
        else
            EmitQuestAttempt(lines, target, context, new HashSet<int>(), "                        ", "req.ID", registeredQuestIDs);
    ...
    {
        HashSet<int> leftIDs = (left.SimpleRewards ?? new List<SimpleReward>())
            .Where(reward => reward != null && reward.ID > 0)
            .Select(reward => reward.ID)
            .Concat((left.Rewards ?? new List<ItemBase>())
                .Where(reward => reward != null && reward.ID > 0)
                .Select(reward => reward.ID))
            .ToHashSet();
        if ((right.SimpleRewards ?? new List<SimpleReward>())
            .Any(reward => reward != null && leftIDs.Contains(reward.ID)))
            return true;
        return (right.Rewards ?? new List<ItemBase>())
            .Any(reward => reward != null && leftIDs.Contains(reward.ID));
    }
