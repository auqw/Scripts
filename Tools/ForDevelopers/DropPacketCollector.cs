...
public sealed class DropPacketCollector
{
    ...
    {
        int itemID = ReadInt(item, "ItemID", "ID", "iItemID");
        return new(
            itemID > 0 ? itemID : keyedID,
            ReadString(item, "sName", "Name", "strName"),
            ReadInt(item, "bTemp", "Temp", "isTemp") != 0,
    ...
    private static int ReadInt(JToken item, params string[] names)
    {
        foreach (string name in names)
            if (int.TryParse(item[name]?.ToString(), out int value))
                return value;
        return 0;
    }
