...
public class Butler
{
    ...
    public List<IOption> Options = new()
    {
        ...
        new Option<string>(
            "attackPriority",
            "Attack Priority",
            "Fill in the monsters that the bot should prioritize (in order), split with a , (comma)."
        ),
        new Option<bool>(
            "copyWalk",
            "Copy Walk",
    ...
    void DoButler()
    {
        ...
        if (
            !int.TryParse(Bot.Config!.Get<string>("roomNumber"), out int roomNr)
            && Bot.Config!.Get<bool>("lockedMaps")
        )
        {
            Core.Logger(
                "Please provide a room number for the bot to use whilst searching locked zones",
                messageBox: true
            );
