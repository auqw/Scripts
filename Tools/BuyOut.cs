...
public class BuyOut
{
    ...
    public List<IOption> Options = new()
    {
        new Option<string>(
            "map_names",
            "Map Names",
            "Map Name of Shop to load, - for default, comma to delimit",
            "battleon,-"
        ),
        ...
        new Option<string>(
            ...
            "Mode [all]: \t\tYou get all the items from shop, even non-AC ones and expensive AC ones, if any.",
            "click here"
        ),
        new Option<string>(
            " ",
            "Mode Explanation [zero_ac_only]",
            "Mode [zero_ac_only]: \tYou get all the AC tagged items from the shop that are 0 ACs.",
            "click here"
        ),
        new Option<string>(
            " ",
            "Mode Explanation [zero_ac_and_gold]",
            "Mode [zero_ac_and_gold]: \tYou get all the AC tagged items from the shop that are 0 ACs, and non-AC items.",
            "click here"
        ),
        new Option<string>(
            " ",
            "Mode Explanation [gold_only]",
            "Mode [zero_ac_and_gold]: \tYou get all the gold items from the shop.",
            "click here"
        ),
        new Option<string>("blank", " ", "", ""),
    };
    ...
    public void Buy()
    {
        List<string> map_names =
            Bot.Config?.Get<string>("map_names")?.Split(",").ToList() ?? new List<string>();
        List<int> shop_ids =
            Bot.Config?.Get<string>("shop_ids")?.Split(",").Select(id => int.Parse(id)).ToList()
            ?? new List<int>();
        ...
        foreach ((string map_name, int shop_id) in map_names.Zip(shop_ids))
        {
            ...
            foreach (ShopItem item in shop_items)
            {
                ...
                else if (item.Coins)
                {
                    if (
                        (mode == BuyEnums.all)
                        || (mode <= BuyEnums.zero_ac_and_gold && item.Cost == 0)
                    )
                    {
